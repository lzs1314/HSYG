$(document).ready(function(){
	var $J_code = $(".J_code"),
		...
		$file_sp = $("#file_sp"),
		$img_sp = $("#img_sp"),
		name,code,lxname,qyname,yynum,frname,//定义文本框项目
	...
	/********焦点获得时**********/
	$(".J_code,.J_qyname,.J_lxname,.J_yynum,.J_frname").focus(function(){
	...
	$J_code.blur(function(){
		if ($J_code.val() == "") {
        	$(this).next().show();
        	$(this).removeClass("aI_blue");
        	$(this).parent().next().removeClass("aI_right").removeClass("aI_wrong").html("&nbsp;");
            return true;
        }else if(!enReg.test($J_code.val())){
    		$(this).parent().next().addClass("aI_wrong").removeClass("aI_right");
			$(this).addClass("aI_red").removeClass("aI_blue");
            $(this).parent().next().show().html("请输入正确的推荐码");
            return false;
   		}else{
        	$(this).parent().next().addClass("aI_right").removeClass("aI_wrong");
			$(this).removeClass("aI_blue").removeClass("aI_red");
